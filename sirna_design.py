...
# 第4步: BLAST
# ─────────────────────────────────────────────
def blast_sequence(seq, name="siRNA", database="refseq_rna", timeout_total=180):
    ...
    try:
        ...
        return {'error': str(e), 'total': 0, 'human_sun1': 0, 'human_offtarget': 0, 'other_sun1': 0}

    m = re.search(r'RID\s*=\s*(\S+)', text)
    if not m:
        return {'error': 'No RID', 'total': 0, 'human_sun1': 0, 'human_offtarget': 0, 'other_sun1': 0}
    rid = m.group(1)
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            if '<BlastOutput>' in t2:
                ...
                hits_ident = re.findall(r'<Hsp_identity>([^<]+)</Hsp_identity>', t2)

                stats = {'total': 0, 'human_sun1': 0, 'human_offtarget': 0, 'other_sun1': 0}
                for i in range(len(hits_def)):
                    e = float(hits_e[i]) if i < len(hits_e) else 999
                    ident = int(hits_ident[i]) if i < len(hits_ident) else 0
                    # 只统计E值<1的显著匹配（19nt完美匹配的E值约0.06）
                    if e >= 1:
                        continue
                    stats['total'] += 1
                    is_sun1 = 'SUN1' in hits_def[i] or 'Sad1' in hits_def[i] or 'UNC84' in hits_def[i]
                    if is_sun1:
                        stats['human_sun1'] += 1
                    else:
                        stats['human_offtarget'] += 1
                return stats
        except:
            continue

    return {'error': 'timeout', 'total': 0, 'human_sun1': 0, 'human_offtarget': 0, 'other_sun1': 0}
...
def generate_report(selected, gene_name, transcript_id, cds_len, output_path):
    ...
    for i, c in enumerate(selected):
        ...
        if blast_info.get('error'):
            ...
            blast_str = f'人SUN1: {blast_info.get("human_sun1", "?")}  脱靶: {blast_info.get("human_offtarget", "?")}'
...
def design_sirnas(gene_name=None, species="Homo sapiens", transcript_id=None,
    ...
    for i, c in enumerate(selected):
        ...
        result = blast_sequence(c['ss'])
        c['blast'] = result
        if 'error' in result:
            print(f"    ⚠ {result['error']}")
        else:
            print(f"    总命中: {result['total']}, 人SUN1: {result['human_sun1']}, "
                  f"人类脱靶: {result['human_offtarget']}, 跨物种SUN1: {result['other_sun1']}")
